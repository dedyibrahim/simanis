...
<div class="section-title">Daftar Legalisasi</div>
<table class="data-table">
    <thead>
        <tr>
            <th>Judul Surat</th>
            <th class="nowrap">Tgl Legalisasi</th>
            <th class="nowrap">No Legalisasi</th>
    ...
    <tbody>
        @forelse ($data as $r)
            <tr class="row-primary{{ empty($r['has_document']) ? ' missing-document' : '' }}">
                <td>{{ $r['judul_surat'] }}</td>
                <td class="nowrap">{{ $r['tgl_surat'] }}</td>
                <td class="nowrap">{{ $r['no_legalisasi'] }}</td>
                <td>{{ $r['pengambil'] }}</td>
            </tr>
        ...
        @empty
            <tr>
                <td colspan="4" class="empty-state">Tidak ada data legalisasi untuk periode ini.</td>
            </tr>
        @endforelse
    </tbody>
</table>
